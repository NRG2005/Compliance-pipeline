"""
T3: Risk Score
- Converts spreadsheet or event data into normalized JSON
- Runs local data-quality and risk heuristics
- Optionally asks Phi-4-mini on Ollama for a structured risk verdict
"""
import asyncio
import json
import math
...
def _call_ollama_sync(prompt: str) -> Dict[str, Any]:
    payload = {
        "model": OLLAMA_MODEL,
        "prompt": prompt,
        "stream": False,
...
async def analyze_risk_from_records(
    ...
    if use_llm:
        ...
        try:
            llm_result = _normalize_llm_result(await _call_ollama(prompt), heuristic_score)
...
async def calculate_risk_score(
    ...
    """
    Calculates a structured T3 risk result for a transaction.
